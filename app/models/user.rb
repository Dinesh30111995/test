class User < ApplicationRecord
  ...
  def mention_data
    { name: user_name, address: address}
  end
  ...
  def self_activities(filters, page_no=1)
    follow_ids = Follow.where(follower_id: id).pluck(:id)
    # query_params = build_query_params(filters, collections.pluck(:id), likes.pluck(:id), follow_ids, bids.pluck(:id))
    query_params = build_query_params(filters, collections.pluck(:id), likes.pluck(:id), follow_ids, bids.pluck(:id))
    activities(query_params, page_no)
  end
  ...
  def swap_collection_list(collection_type, collection)
    collections
      .where(collection_type: collection_type)
      .where('token is not NULL')
      .where.not(id: Swap.where(requestor: self, owner_collection: collection).pluck(:requestor_collection_id))
  end
