		...
		if (input.files && input.files[0]) {
			...
			reader.onload = function(e) {
				if (imgExt.includes(ftype)) {
					...
					previewSection.hide();
					previewSection.fadeIn(650);
					imagePreview.html(`<object data="${URL.createObjectURL(input.files[0])}" type="application/pdf" width="300" height="200">
				  </object>`);
					imagePreview.css({ 'height': '225px' });
				}else {
					return toastr.error('Invalid file type!');
 ...
 //    });
  ...
  //   }
	...
	$("#promote-post").click(function(){
		$(".open-promote-post").toggleClass("main-btn");
		if($(".icon-element").hasClass("change")) {
			$(".icon-element").removeClass("change");
			// $('#collection_is_promoted').val(false)
		}else {
			// $('#collection_is_promoted').val(true)
			$(".icon-element").addClass("change");
		}
	});
