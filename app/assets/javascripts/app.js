!(function (s) {
  ...
  function a(e) {
      document.getElementById("header-lang-img") &&
          ("eng" == e
              ? (document.getElementById("header-lang-img").src = "assets/img/flags/us.jpg")
              : "sp" == e
              ? (document.getElementById("header-lang-img").src = "assets/img/flags/spain.jpg")
              : "gr" == e
              ? (document.getElementById("header-lang-img").src = "assets/img/flags/germany.jpg")
              : "it" == e
              ? (document.getElementById("header-lang-img").src = "assets/img/flags/italy.jpg")
              : "ru" == e && (document.getElementById("header-lang-img").src = "assets/img/flags/russia.jpg"),
  ...
  s("#side-menu").metisMenu(),
      ...
      s(document).ready(function () {
          ...
          0 < s("#sidebar-menu").length &&
              ...
              ((e -= 300), s(".simplebar-content-wrapper").animate({ scrollTop: e }, "slow"));
      }),
      ...
      window.sessionStorage && ((e = sessionStorage.getItem("is_visited")) ? (s(".right-bar input:checkbox").prop("checked", !1), s("#" + e).prop("checked", !0), o(e)) : sessionStorage.setItem("is_visited", "light-mode-switch")),
      s("#light-mode-switch, #dark-mode-switch, #rtl-mode-switch").on("change", function (e) {
          o(e.target.id);
      }),
      null != t && t !== n && console.log("lang", t),
      a(t),
      ...
      // Edition collections Starts
      var edd_table= s('#edition-collections').DataTable( {
        "bInfo" : false,
        scrollResize:     true,
        lengthChange:     false,
        searching:        false,
      ...
      s('#edition_add').click(function () {
        var ids = s.map(edd_table.rows('.selected').data(), function (item) {
            return item[1]
        });
        s('#selected_edition_items').val(ids)
        s('#edition_drops_form').submit()
      });
      // Edition collections Ends

      // Reserve Starts
      var res_table= s('#reserve-collections').DataTable( {
        "bInfo" : false,
        scrollResize:     true,
        lengthChange:     false,
        searching:        false,
      ...
      s('#reserve_add').click(function () {
        var ids = s.map(res_table.rows('.selected').data(), function (item) {
            return item[1]
        });
        s('#selected_reserve_items').val(ids)
        s('#reserve_drops_form').submit()
      });
      // Reserver Ends
