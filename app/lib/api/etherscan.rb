module Api
  class Etherscan
    ZERO = 0.0
    COINGECKO_IDS = {'weth': 'weth', 'wbnb': 'wbnb', 'wmatic': 'wmatic'}
    ...
    def self.get_all_price(fiat='usd')
      ids = COINGECKO_IDS.map { |_, id| id }.join(",")
      uri = URI.parse(Rails.application.credentials.config[:coingecko_url] + "?ids=#{ids}&vs_currencies=#{fiat}")
      request = Net::HTTP::Get.new(uri)
      request.content_type = "application/json"
      req_options = {
          use_ssl: uri.scheme == "https",
